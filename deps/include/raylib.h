...
typedef struct Vector2
{
    ...
    ~Vector2() = default;
    void Add(const Vector2 &v);              // v1.Add(v2)
    void Sub(const Vector2 &v);              // v1.Sub(v2)
    void Scale(const float n);               // v1.Scale(n)
    Vector2 Rotate(const float angle) const; // v1.Rotate(angle)

    float Magnitude() const;        // v1.Magnitude()
    float MagnitudeSquared() const; // v1.MagnitudeSquared()

    Vector2 &Normalize();       // v1.Normalize()
    Vector2 UnitVector() const; // v1.UnitVector()
    Vector2 Normal() const;     // n = v1.Normal()

    float Dot(const Vector2 &v) const;   // v1.Dot(v2)
    float Cross(const Vector2 &v) const; // v1.Cross(v2)

    Vector2 &operator=(const Vector2 &v);    // v1 = v2
    bool operator==(const Vector2 &v) const; // v1 == v2
    bool operator!=(const Vector2 &v) const; // v1 != v2

    Vector2 operator+(const Vector2 &v) const; // v1 + v2
    Vector2 operator-(const Vector2 &v) const; // v1 - v2
    Vector2 operator*(const float n) const;    // v1 * n
    Vector2 operator/(const float n) const;    // v1 / n
    Vector2 operator-();                       // -v1

    Vector2 &operator+=(const Vector2 &v); // v1 += v2
    Vector2 &operator-=(const Vector2 &v); // v1 -= v2
    Vector2 &operator*=(const float n);    // v1 *= n
    Vector2 &operator/=(const float n);    // v1 /= n
} Vector2;
...
void Vector2::Scale(const float n)
{
    x *= n;
    y *= n;
}
